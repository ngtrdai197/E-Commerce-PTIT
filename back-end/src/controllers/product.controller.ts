    ...
    */
    ...
    public async unLink(req: Request, res: Response): Promise<any> {
        try {
            ...
            return res.status(400).send({ isDeleted: false, message: 'Không tìm thấy sản phẩm để cập nhật' });
        } catch (error) {
            throw error;
        }
    }
    ...
    public async delete(req: Request, res: Response): Promise<any> {
        try {
            ...
            if (product) {
                ...
                if (result) {
                    return res.status(400).json({ statusCode: 400, message: `Không thể xóa sản phẩm này, vì nó đang nằm trong 1 giỏ hàng.` });
                }
